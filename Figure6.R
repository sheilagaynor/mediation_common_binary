library(ggplot2)
setwd("~/Binary1000/")
binTruth <- data.frame(read.delim("~/binSimTruth.txt", stringsAsFactors=FALSE))
names(binTruth) <- c("prev", "trueIE", "trueDE")
table_out <- list.files("~/Desktop/Xihong/Final_Simulations/Binary1000/")
...
names(figData1000) <- c("prev", "sdIE", "sdDE", "propIE", "propDE",
                        "deltaIE", "deltaDE","bootIE_se",
                        "bootDE_se", "bootIE_mad", "bootDE_mad")
figData1000 <- merge(binTruth,figData1000, by="prev")
...
#For 500 sample
setwd("~/Binary500/")
table_out <- list.files("~/Binary500/")
...
names(figData500) <- c("prev", "sdIE", "sdDE", "propIE", "propDE",
                       "deltaIE", "deltaDE","bootIE_se",
                       "bootDE_se", "bootIE_mad", "bootDE_mad")
figData500 <- merge(binTruth,figData500, by="prev")
...
g1 <- ggplot(plotData, aes(x = figData500.prev, y = value, colour = (Sample))) + 
  ...
  xlim(0.1,0.75) +  ylim(0.015,0.05) +
  theme(panel.grid.major = element_blank(),
        panel.border = element_rect(colour = "black", fill=NA, size=0.75),
        panel.grid.minor = element_blank(),
        panel.background = element_blank(), 
        axis.line = element_line(colour = "black"),
        plot.title = element_text(hjust = -0.23, face = 'bold')) 
...
g3 <- ggplot(plotData, aes(x = figData500.prev, y = value, colour = Sample)) + 
  ...
  xlim(0.1,0.75) +  ylim(0.11,0.35) + 
  theme(panel.grid.major = element_blank(),
        panel.border = element_rect(colour = "black", fill=NA, size=0.75),
        panel.grid.minor = element_blank(),
        panel.background = element_blank(), 
        axis.line = element_line(colour = "black"),
        plot.title = element_text(hjust = -0.19, face = 'bold'),
...
g2 <- ggplot(plotData, aes(x = figData1000.prev, y = value, colour = (Sample))) + 
  ...
  theme(panel.grid.major = element_blank(),
        ...
        panel.background = element_blank(), 
        axis.line = element_line(colour = "black"),
        plot.title = element_text(hjust = -.23, face = 'bold')) 
...
ggsave('~/Figure6.eps', plot=grid.arrange(arrangeGrob( g1 + theme(legend.position="none"),
